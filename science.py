...
for entry in news_feed.entries[:5]:
    ...
    try:
        ...
        html_response = requests.get(target_url, headers=headers, timeout=15)
    
        
        if html_response.status_code == 200:
            print(f"{i}.成功进入网站，news-please正在提取纯净正文...")
            
            article = NewsPlease.from_html(html_response.text, url=target_url)
        
            
            if hasattr(article, 'maintext') and article.maintext:
                ...
                print("哎呀，这篇网页里没找到正文文字，跳过喵~")
        else:
            print(f"被网站无情拒绝啦，状态码：{html_response.status_code}，只能跳过了哦...")
        
    except Exception as e:
        print(f"抓取这篇新闻时发生了一点小意外：{e}，咱们继续下一篇！")
    i = i+1
